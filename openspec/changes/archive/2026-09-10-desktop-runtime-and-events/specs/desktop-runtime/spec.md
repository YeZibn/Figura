...
The development workspace SHALL provide one documented command that starts the
Gateway-mode client and its local Python Gateway as one supervised workflow.
Startup SHALL wait for a compatible Gateway health response before reporting the
client ready, SHALL use the `agent` Conda environment by default, and SHALL
