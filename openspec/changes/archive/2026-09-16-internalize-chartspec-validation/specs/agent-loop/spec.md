...
select OCR, geometry, or layout tools for unresolved evidence, and use
...
`validate_spec` tool call or prescribe `inspect_chart_layout` as a universal
precondition. When a generated chart creates a pending review obligation, the
model-visible context SHALL identify the candidate references and required
next action as bounded structured data.
