...
**Reason**: 该要求名为 measurement review gate，容易继续维持已删除的独立决策生命周期；其有用的引用校验和非阻塞行为由下方的新要求明确表达。

**Migration**: 测量只提供候选 evidence。组装按新 requirement 校验实际传入的 refs 与来源范围，不再创建测量审核或决策门禁。
