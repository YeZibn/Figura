...
between the supported OpenAI relay, Qwen, and DeepSeek providers while secrets
remain server-owned.
...
- **THEN** the response identifies `openai`, `qwen`, and `deepseek` provider
  names when supported by the deployment, safe availability states, and the
  configured default provider without secrets
