...
Figura SHALL expose an internal commit boundary that appends only supported, bounded, versioned execution facts and advances the same Run's checkpoint atomically. Record sequences SHALL be unique and increasing within a Run. A commit SHALL reject a stale expected checkpoint, an invalid record reference, an unsupported payload kind or version, or a terminal Run without partially advancing records, checkpoint, or events. This change SHALL support input, text-only model-response, and final-answer facts; it SHALL reject a model response containing tool calls or provider-private continuation rather than silently discarding either.
...
- **THEN** Figura appends the response as the next record and advances the checkpoint to a final-answer action in the same commit
...
#### Scenario: Response contains unsupported private or tool data
- **WHEN** a model response includes tool calls or provider-private continuation that this change cannot durably represent
