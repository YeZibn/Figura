...
def test_agent_repl_uses_advisory_chart_default(monkeypatch):
    ...
    assert "pass_with_warning" in captured["system"]
    assert "暂存预览" in captured["system"]
    assert "发布状态" in captured["system"]
    assert "verification.status" in captured["system"]
    assert "assemble_spec" in captured["system"] and "render_chart" in captured["system"]
    assert "无工具 VLM 检查" in captured["system"]
