...
export function GeneratedChartView({ artifact, loader, onPreview }: { artifact: GeneratedChartReference; loader: PreviewResourceLoader | null; onPreview?: PreviewOpener }) {
  ...
  const verificationStatus = artifact.verification?.status || artifact.status
  const hasPreview = Boolean(artifact.imageUrl || artifact.previewResource)
  const status = artifact.artifactId
    ? verificationStatus === 'pass_with_warning' ? 'warning' : 'available'
    : verificationStatus === 'fail' ? 'failed'
      : verificationStatus === 'unavailable' || !hasPreview || imageFailed ? 'unavailable' : 'pending'
  const statusLabel = status === 'available' ? '已发布'
    : status === 'warning' ? '已发布·有警告'
      : status === 'pending' ? '仅预览·发布未完成'
        : status === 'failed' ? '验证未通过·仅可预览' : '暂不可用'
  const metadata = [
  ...
  const download = async () => {
    if ((!artifact.downloadUrl && !artifact.artifactId) || downloading) return
    setDownloading(true)
    setDownloadError('')
    try {
      let objectUrl = artifact.downloadUrl || ''
      let temporary = false
      if (loader && artifact.artifactId && artifact.previewResource) {
