...
const required = ['src/App.tsx', 'src/api/client.ts', 'src/api/workspace.ts', 'src/api/mockClient.ts', 'src/api/mock/fixtures.ts', 'src/api/gatewayClient.ts', 'src/api/gateway/transport.ts', 'src/api/gateway/mappers.ts', 'src/api/gateway/runStream.ts', 'src/api/gateway/evaluationResource.ts', 'src/domain/run/controller.ts', 'src/components/common.tsx', 'src/components/dialogs.tsx', 'src/components/evaluation.tsx', 'src/components/preview.tsx', 'src/components/run.tsx', 'src/components/types.ts', 'src/components/workspace.tsx', 'src/types/protocol.ts', 'src/runtime.ts', 'src/previewResources.ts', 'src/styles/global.css', 'src/styles/tokens.css', 'src/styles/foundation.css', 'src/styles/workspace.css', 'src/styles/evaluation.css', 'src/styles/attachment-preview.css', 'src/styles/dialogs.css', 'src/styles/run.css', 'src/styles/responsive.css', 'index.html', 'scripts/dev-gateway.mjs', 'scripts/tauri-dev-gateway.mjs', 'scripts/launcher-smoke.mjs', 'scripts/launcher-lifecycle-smoke.mjs']
...
const source = app + gateway + workspaceApi + gatewayTransport + gatewayMappers + gatewayStream + evaluationResourceApi + controller + domain + components + mockFixtures + client + attachmentHelpers + stylesheet + runtime + previewResources + mock + launcher + tauriLauncher + tauriConfig + tauriEntrypoint + tauriPackage + gatewayServer + pythonPackage + readme
...
checks.push(['measurement repair required event', 'measurement_repair_required'], ['measurement repair rejected event', 'measurement_repair_rejected'], ['measurement repair exhausted event', 'measurement_repair_exhausted'], ['measurement decision event', 'measurement_decision_required'], ['measurement focus event', 'measurement_focus_applied'], ['measurement evidence selection event', 'measurement_evidence_selected'], ['measurement repair event kinds', 'measurementRepairEventKinds'], ['measurement repair summary', 'measurementRepairSummary'], ['measurement repair required label', '需要定向补充'], ['measurement repair rejected label', '定向补充被拒绝'], ['measurement repair exhausted label', '定向补充次数已用尽'], ['measurement decision label', '等待主 Agent 选择测量证据'], ['measurement repair mock sample', 'run_mock_repair'], ['measurement repair replay merge', 'mergeEvents'], ['measurement repair reconnect', 'scheduleReconnect'])
...
checks.push(['decision timeline projector', 'projectDecisionTimeline'], ['decision unit correlation', 'transitionIndexes'], ['decision unit legacy fallback', 'legacyUnitId'], ['review subcheck grouping', 'review_subcheck'], ['decision timeline mock fixture', 'mockMeasurementUnit'], ['evaluation shared timeline view', 'showSummary={false}'])
