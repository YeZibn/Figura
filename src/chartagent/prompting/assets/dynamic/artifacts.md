...
对于 `measurement_status`、`measurement_reference` 和 `measurement_issues`，它们只是当前 run 的有界索引：`accepted` 才是可供 `assemble_spec` 引用的候选，其他状态必须回到原生 tool observation 读取完整 issue 和视觉证据。artifact index 不能自行升级测量状态，也不能替代服务端 measurement gate。
