...
from .tools.result import DispatchedObservation, GeneratedImage
...
def _sensor_result(path: Path, chart_type: ChartType) -> tuple[Mapping[str, Any] | None, str | None]:
    ...
    try:
        if chart_type is ChartType.BAR:
            from .tools.chart.geometry import measure_bars
            sensor = measure_bars
        elif chart_type is ChartType.LINE:
            from .tools.chart.line import extract_line_series
            sensor = extract_line_series
        elif chart_type is ChartType.PIE:
            from .tools.chart.pie import extract_pie_slices
            sensor = extract_pie_slices
        else:
            from .tools.chart.scatter import extract_scatter_points
...
class ChartReviewManager:
    ...
    def review_tool(self):
        ...
        def review_generated_chart(candidate_id: str, review_id: str, decision: dict | None = None) -> dict[str, Any]:
            candidate = self.get(candidate_id, review_id)
            if candidate is None:
                return {"error": "candidate or review context is not authorized"}
            result = self.process(candidate, decision=decision)
            return {"candidate": result.safe_metadata(), "review": result.review.to_dict() if result.review else None}
